...
> [!TIP]
> Public GET endpoints support standard `Accept: application/json` headers and include CORS support for client-side JavaScript applications.

---

## Turnstile Verification

Certain POST endpoints (such as contact submission or rate-sensitive forms) may utilize Cloudflare Turnstile token validation via the `cf-turnstile-response` header or POST parameters.
...
| `404 Not Found` | `NOT_FOUND` | The specified endpoint or resource does not exist. |
| `405 Method Not Allowed` | `METHOD_NOT_ALLOWED` | HTTP method not permitted for this endpoint. |
| `500 Internal Error` | `SERVER_ERROR` | An uncaught server-side exception occurred. |
